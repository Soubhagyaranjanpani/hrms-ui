...
const Skill = () => {
  ...
  const filteredSkills = skills
    .filter((d) =>
      d.skillName?.toLowerCase().includes(debouncedSearch.toLowerCase())
    )
    .sort((a, b) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const err = validate("skillName", formData.skillName);
    setTouched({ skillName: true });
    setErrors({ skillName: err });
    if (err) {
    ...
    setSubmitting(true);
    try {
      const nameTrimmed = formData.skillName.trim();
      const descriptionTrimmed = formData.description?.trim() || "";
      let url, method, payload;

      if (editMode) {
        url = `${BASE_URL}/api/skills/update`;
        method = axios.put;
        payload = { id: selectedSkill.id, name: nameTrimmed, description: descriptionTrimmed };
      } else {
        url = `${BASE_URL}/api/skills/create`;
        method = axios.post;
        payload = { name: nameTrimmed, description: descriptionTrimmed };
      }

      const res = await method(url, payload, axiosConfig);
      if (res.data?.status === 200) {
        toast.success("Success", editMode ? "Skill updated" : "Skill created");
        resetForm();
        setView("list");
        fetchSkills();
      } else {
  ...
  const confirmStatusChange = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.put(`${BASE_URL}/api/skills/status/${id}`, null, axiosConfig);
      if (res.data?.status === 200) {
        toast.success("Status Updated", "Skill status changed");
        fetchSkills();
      } else {
  ...
  if (loading && view === "list" && skills.length === 0) {
    ...
    <>
      <div className="emp-root">
          ...
          {view === "form" ? (
            ...
            <>
              <div className="emp-header-left">
                <div>
                  <h1 className="emp-title">Skill Directory</h1>
        ...
        {view === "list" ? (
                    ...
                    {currentSkills.length > 0 ? (
                      currentSkills.map((skill, idx) => (
                        <tr key={skill.id} className="emp-row">
                          ...
                          <td>
                            <div className="emp-name">{skill.skillName || "—"}</div>
                          </td>
                          <td>
                            <div
          ...
          /* ========== FORM VIEW – EXACTLY LIKE DESIGNATION PAGE ========== */
