...
const LeaveTypes = () => {
  ...
  const [formData, setFormData] = useState({ name: "", maxDaysPerYear: 1 });
  ...
  const resetForm = () => {
    setFormData({ name: "", maxDaysPerYear: 1 });
    setErrors({});
    setTouched({});
    setEditMode(false);
                    ...
                    {currentItems.length > 0 ? (
                      currentItems.map((item, idx) => (
                        <tr key={item.id} className="emp-row">
                          ...
                          <td className="emp-name">{item.name || "—"}</td>
            ...
            <form onSubmit={handleSubmit} noValidate className="emp-form-compact">
              <div className="emp-form-section-compact">
                ...
                <div className="emp-form-grid-3col">
                  ...
                  {/* Max Days Per Year */}
                  <div className={`emp-field-compact ${isFieldErr('maxDaysPerYear') ? 'has-error' : ''} ${isFieldOk('maxDaysPerYear') ? 'has-ok' : ''}`}>
                    <label>Max Days Per Year <span className="req">*</span></label>
                    <input
                      type="number"
                      min="1"
                      max="365"
          ...
          <div className="emp-modal-overlay" onClick={() => setShowStatusModal(false)}>
            <div className="emp-modal" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="emp-modal-actions">
                ...
                <button className="emp-modal-confirm" onClick={confirmStatusChange} disabled={statusUpdating}>
