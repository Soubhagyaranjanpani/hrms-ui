...
export default function PayrollDashboard({ user }) {
  ...
  return (
    <div style={{ padding: '24px', background: 'var(--bg-page)', minHeight: '100vh', animation: 'payFade .4s ease' }}>
      <style>{`
        @keyframes payFade { from{opacity:0;transform:translateY(16px)} to{opacity:1;transform:translateY(0)} }
        .pay-card { background:var(--card-bg); border:1px solid var(--border-light); border-radius:18px; overflow:hidden; }
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
          <select
            ...
            style={{ padding: '9px 14px', border: '1.5px solid var(--border-medium)', borderRadius: 11, fontSize: 13, fontFamily: "'DM Sans',sans-serif", color: 'var(--text-primary)', background: 'var(--bg-white)', outline: 'none', cursor: 'pointer' }}
          >
        ...
        <div style={{ background: 'linear-gradient(135deg,#1e1b4b,#312e81)', borderRadius: 16, padding: '16px 20px', marginBottom: 22, display: 'flex', alignItems: 'flex-start', gap: 14 }}>
          ...
          {momChange !== null && (
            <div style={{ marginLeft: 'auto', textAlign: 'right', flexShrink: 0 }}>
              <div style={{ fontSize: 22, fontWeight: 700, color: parseFloat(momChange) >= 0 ? '#34d399' : '#f87171', fontFamily: "'Sora',sans-serif" }}>
                {parseFloat(momChange) >= 0 ? <FaArrowUp size={14} /> : <FaArrowDown size={14} />} {Math.abs(momChange)}%
              </div>
              <div style={{ fontSize: 11, color: '#818cf8' }}>vs last month</div>
            </div>
          )}
        </div>
      )}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16, marginBottom: 22 }}>
        {statCards.map(({ label, value, sub, icon, grad, light }) => (
          <div key={label} className="pay-card" style={{ padding: 22, position: 'relative', overflow: 'hidden' }}>
            ...
            <div style={{ fontSize: 28, fontWeight: 700, fontFamily: "'Sora',sans-serif", color: 'var(--text-primary)', marginBottom: 3 }}>{value}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18, marginBottom: 22 }}>
        ...
        <div className="pay-card" style={{ padding: 22 }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
            <div>
              <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700 }}>Payroll Trend</h3>
              <p style={{ margin: 0, fontSize: 11, color: 'var(--text-muted)' }}>Net salary outflow · last 6 months</p>
            </div>
            ...
            <div>
                ...
                {trend.map((t) => {
                  ...
                  return (
                    <div key={t.yearMonth} style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
                      <div style={{ fontSize: 11, fontWeight: 700, color: isSelected ? '#6366f1' : 'var(--text-primary)' }}>
                        {fmtK(t.netPayroll)}
                      </div>
                      <div
                        className="trend-bar"
              ...
              <div style={{ display: 'flex', justifyContent: 'center', gap: 16, marginTop: 8, paddingTop: 8, borderTop: '1px solid var(--border-light)' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <span style={{ fontSize: 11, color: 'var(--text-muted)' }}>Employee Count</span>
                </div>
              </div>
            </div>
          )}
