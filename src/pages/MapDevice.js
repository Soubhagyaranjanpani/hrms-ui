...
import {
  ...
  FaBuilding, FaUsers, FaCalendarAlt, FaToggleOn,
...
const MapDeviceToBranch = () => {
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this mapping?')) {
      setDeviceMappings(deviceMappings.filter(m => m.id !== id));
  ...
  // ─── Pagination ──────────────────────────────────────────
  const filteredMappings = deviceMappings;
        ...
        <div style={styles.card}>
          {deviceMappings.length === 0 ? (
            <div style={{ textAlign: 'center', padding: '60px 20px' }}>
              ...
              <h3 style={{ fontSize: '18px', fontWeight: '600', color: '#475569', marginBottom: '8px' }}>No devices mapped yet</h3>
              <p style={{ fontSize: '14px', color: '#94a3b8', marginBottom: '16px' }}>Click "Map Device" to assign devices to branches</p>
              <button style={styles.btnPrimary} onClick={openForm}>
                <FaPlus size={13} /> Map Device
              </button>
            </div>
